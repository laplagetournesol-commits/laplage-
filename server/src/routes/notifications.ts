...
import { sendWhatsAppConfirmation, sendWhatsAppAdminNotification, sendWhatsAppGuestPayment, sendWhatsAppGuestConfirm } from '../lib/whatsapp';
...
async function sendConfirmationEmail(
  ...
  const { data: resa } = await supabase
    .from(table)
    .select('date, time, time_slot, guest_count, guest_email, guest_name, special_requests, qr_code, guest_payment_requested, guest_confirmed, guest_payment_token, total_price')
    .eq('id', reservationId)
    .single();
  ...
  // Variante "pour un ami" SANS paiement, pas encore confirmée → DEMANDE DE CONFIRMATION de venue.
  if (madeByBeach && !(resa as any)?.guest_payment_requested && !(resa as any)?.guest_confirmed && (resa as any)?.guest_payment_token) {
    const APP_URL = process.env.PUBLIC_WEB_URL ?? 'https://laplagetournesols.com';
    const confirmUrl = `${APP_URL}/confirm/${(resa as any).guest_payment_token}`;
    const firstName = String((resa as any)?.guest_name ?? '').trim().split(/\s+/)[0] || '';
    try {
      await new Resend(process.env.RESEND_API_KEY).emails.send({
        from: fromEmail,
        to,
        subject: 'Confirmez votre venue — La Plage Tournesol',
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color:#3D434F;">La Plage Tournesol ☀️</h2>
            <p>Bonjour ${firstName},</p>
            <p>Une place vous a été réservée à La Plage Tournesol${formattedDate ? ` pour le <strong>${formattedDate}</strong>` : ''}.</p>
            <p>Merci de <strong>confirmer votre venue</strong> en un clic :</p>
            <div style="text-align:center;margin:26px 0;">
              <a href="${confirmUrl}" style="background:#3D434F;color:#fff;text-decoration:none;padding:14px 28px;border-radius:10px;font-weight:700;display:inline-block;">Je confirme ma venue</a>
            </div>
            <p style="color:#888;font-size:13px;">Ou copiez ce lien : ${confirmUrl}</p>
            <p style="color:#888;font-size:12px;">La Plage Tournesol</p>
          </div>
        `,
      });
    } catch (err) {
      console.error('Erreur envoi email confirmation invité:', err);
    }
    return;
  }
...
async function sendWhatsAppToUser(
  ...
  const { data: reservation } = await supabase
    .from(table)
    .select('guest_name, guest_phone, guest_payment_requested, guest_confirmed, guest_payment_token, date, special_requests')
    .eq('id', reservationId)
    .single();
  ...
  // Variante "pour un ami" SANS paiement, pas encore confirmée : template de confirmation de venue.
  if ((reservation as any)?.special_requests === 'Réservation admin' && !(reservation as any)?.guest_payment_requested && !(reservation as any)?.guest_confirmed && (reservation as any)?.guest_payment_token) {
    const dateLabel = (reservation as any)?.date
      ? new Date(`${(reservation as any).date}T00:00:00`).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })
      : '';
    const r = await sendWhatsAppGuestConfirm({
      toPhoneE164: toPhone,
      firstName,
      bookerName: 'La Plage Tournesol',
      dateLabel,
    ...
    if (!r.ok) console.error('Erreur WhatsApp confirmation invité:', r.error);
    return;
  }
